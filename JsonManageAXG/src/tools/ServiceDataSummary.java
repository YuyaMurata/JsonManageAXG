...
public class ServiceDataSummary {
    ...
    public static void main(String[] args) throws AISTProcessException {
        ...
        try (PrintWriter pw = CSVFileReadWrite.writerSJIS("PC200_サービス_202001.csv")) {
            ...
            pw.println("車両ID,作番,受注日,SMR,納入からの経過日,作業形態,作業形態名称,請求金額,概要,作業コード,作業名,指示工数,請求工数");
            objex.keySet().stream()
                    .filter(sid -> sid.split("-")[1].equals("10"))
                    .map(sid -> objex.getAnalize(sid))
                    .filter(a -> a.get("受注") != null)
                    .forEach(a -> {
                        //サービスデータ
                        a.get("受注").entrySet().stream()
                                .filter(e -> a.getSBNWork(e.getKey()) != null) //作業がないサービスの除外
                                .forEach(e -> {
                                    Map<String, List<String>> w = a.getSBNWork(e.getKey());
                                    
                                    w.values().stream().forEach(wv -> {
                                        ...
                                        sb.add(code);
                                        String cdname = e.getValue().get(h.getHeaderIdx("受注", "作業形態名称"));
                                        sb.add(cdname);
                                        ...
                                        //作業情報取得
                                        sb.add(wv.get(h.getHeaderIdx("作業", "作業コード")));
                                        sb.add(wv.get(h.getHeaderIdx("作業", "作業名称")));
                                        
                                        try {
                                            Double si_step = Double.valueOf(wv.get(h.getHeaderIdx("作業", "指示工数")));
                                            Double se_step = Double.valueOf(wv.get(h.getHeaderIdx("作業", "請求工数")));
